...
| Check an exact durable Run | `xparse-cli task status <TASK_ID> --run-id <RUN_ID>` |
| Read one Task result | `xparse-cli task read <TASK_ID> <FILE_OR_RESOURCE> --run-id <RUN_ID>` |
| Export all completed Task results | `xparse-cli task export <TASK_ID> --run-id <RUN_ID> --out-dir ./task-output` |
| Inspect Task failures | `xparse-cli task debug <TASK_ID> --run-id <RUN_ID>` |

Task Runtime requires authentication and exposes the same control-plane routes
in domestic and overseas environments. Its `auto`/`free` modes create the Agent
free-first Task and do not fall back to a paid Task. An environment without
equivalent free Task billing returns `TASK_FREE_MODE_UNAVAILABLE`; stop rather
than switching workflow or billing mode. See [task-runtime.md](task-runtime.md)
before choosing Task commands.
