...
| `--api` value | Behavior |
|---------------|----------|
| _(omitted)_ | Use `auto`; the server selects free or an existing no-new-charge entitlement |
| `auto` | Same as omitted; never starts a newly chargeable paid request without confirmation |
| `free` | Explicitly require the anonymous free API |
...
Device OAuth. Login can expose an already-owned entitlement to `auto`, but it
never authorizes a newly chargeable request. Only explicit `--api paid` permits
that route after user confirmation.
...
# WorkBuddy emits machine-readable errors automatically
xparse-cli --profile workbuddy parse document.pdf
```
...
| Force JSON errors outside WorkBuddy | `xparse-cli --error-format json parse doc.pdf` |
...
| 1 | Local, merge, or exhausted network error | Read the structured error; do not add an Agent retry loop |
| 2 | Parameter error | Check command syntax; fix and retry |
| 3 | API returned structured error | Read `api_code`, `request_id`, and `suggestion` from stderr |
